...
class MixLibCache:
    def __init__(self, inputdim, max_outliers = 12288, max_batch=1024,max_weight_dim_N=12288,max_activatetion_dim=12288):
        self.device = 'cuda'

        self.sigma = torch.zeros((1,1),dtype=torch.float16).to('cuda')  
        self.zeros = torch.zeros((inputdim,max_outliers),dtype=torch.float16).to('cuda')    
        self.sigma[0] = 3
